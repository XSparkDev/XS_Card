 ...
 * Phase 2: POOP - Microsoft provider implementation
 * 
 * Implements IOAuthProvider interface for Microsoft authentication.
 * Follows the same backend-mediated flow as Google and LinkedIn OAuth.
 */
...
const OAUTH_STATE_KEY = 'oauth_pending_state_microsoft';
...
const storeOAuthState = async (state: string): Promise<void> => {
  ...
  try {
    ...
    console.warn('[Microsoft OAuth] Failed to persist OAuth state:', error);
  }
};
...
const retrieveOAuthState = async (): Promise<string | null> => {
  try {
    ...
    console.error('[Microsoft OAuth] Failed to retrieve state:', error);
    return null;
  }
};
...
const clearOAuthState = async (): Promise<void> => {
  ...
  try {
    ...
    console.warn('[Microsoft OAuth] Failed to clear OAuth state:', error);
  }
};
...
class MicrosoftProvider implements IOAuthProvider {
  ...
  async signIn(): Promise<OAuthResult> {
    try {
      ...
      console.log('[Microsoft OAuth] Starting backend-mediated OAuth flow');
      console.log('[Microsoft OAuth] Generated state:', state);

      await storeOAuthState(state);

      const oauthStartUrl = `${backendUrl}/oauth/microsoft/start?state=${state}`;

      console.log('[Microsoft OAuth] Opening browser:', oauthStartUrl);
      ...
      console.log('[Microsoft OAuth] Browser result:', result.type);
      ...
      if (result.type === 'cancel' || result.type === 'dismiss') {
        ...
        if (!remainingState) {
          ...
          console.log('[Microsoft OAuth] Browser closed but state cleared - callback already processed');
        ...
        throw this.createError('user_cancelled', 'Microsoft sign-in was cancelled');
      }
      ...
      if (error.providerId) {
        ...
        console.error('[Microsoft OAuth] Error starting flow:', error);
        await clearOAuthState();
        throw error;
      }
      ...
      console.error('[Microsoft OAuth] Error starting flow:', error);
      await clearOAuthState();
      throw this.createError(error.code || 'unknown_error', error.message || 'Failed to start Microsoft sign-in');
    }
  }

  async handleCallback(url: string): Promise<OAuthResult> {
    try {
      console.log('[Microsoft OAuth] Handling callback URL:', url);
      ...
      if (!receivedState || !storedState || receivedState !== storedState) {
        ...
        console.log('[Microsoft OAuth] Ignoring stale callback. Received state:', receivedState, 'Current state:', storedState);
      ...
      console.log('[Microsoft OAuth] Firebase custom token received, signing in...');

      const userCredential = await signInWithCustomToken(auth, firebaseToken);
      const firebaseUser = userCredential.user;
      const idToken = await firebaseUser.getIdToken();

      console.log('[Microsoft OAuth] Firebase sign-in successful');
      ...
      if (error.code === 'stale_callback') {
        console.log('[Microsoft OAuth] Stale callback ignored, preserving current OAuth state');
...
export const signInWithMicrosoft = async (): Promise<OAuthResult> => {
  return microsoftProvider.signIn();
};

export const handleMicrosoftCallback = async (url: string): Promise<OAuthResult> => {
  return microsoftProvider.handleCallback(url);
};
