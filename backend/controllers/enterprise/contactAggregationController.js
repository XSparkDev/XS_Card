...
async function calculateDepartmentContactsWithDetails(enterpriseId, departmentId) {
    ...
    for (const empDoc of employeesSnapshot.docs) {
        ...
        employeeDetails.push({
            employeeId: empDoc.id,
            userId,
            employeeName: `${employeeData.firstName || employeeData.name || ''} ${employeeData.lastName || employeeData.surname || ''}`.trim(),
...
// Export cache instance for use in other modules (e.g. invalidation from contactController)
