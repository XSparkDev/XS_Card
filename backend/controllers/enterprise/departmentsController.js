...
const { invalidateEnterpriseCache } = require('./contactAggregationController');
const { logActivity, ACTIONS, RESOURCES } = require('../../utils/logger');
const { getEffectiveTemplateForCardCreation } = require('./enterpriseCardTemplateStub');
const { createDefaultCardForUser } = require('../../utils/createDefaultCardForUser');
...
exports.getDepartmentMembers = async (req, res) => {
    try {
        ...
        // Get department members from employees subcollection (user doc has departmentRef, not departmentId)
        const employeesSnapshot = await departmentRef.collection('employees').get();
        const members = [];
        for (const empDoc of employeesSnapshot.docs) {
            const empData = empDoc.data();
            const uid = empDoc.id; // employee doc ID = userId
            const userDoc = await db.collection('users').doc(uid).get();
            const userData = userDoc.exists ? userDoc.data() : empData;
            members.push({
                id: uid,
                name: userData.name || empData.name || '',
                surname: userData.surname || empData.surname || '',
                email: userData.email || empData.email || '',
                title: userData.title || empData.position || '',
                profileImage: userData.profileImage || empData.profileImage || ''
            });
        }
        ...
        // If userId is not provided but email is, try to find user by email
            ...
            if (!usersQuery.empty) {
                ...
                try {
                    ...
                    // Create default card (reuse shared helper - same shape as normal user card creation)
                    await createDefaultCardForUser({
                        db,
                        admin,
                    ...
                    // Create enterprise users subcollection entry (per C1/C2)
