...
const addContactHandler = async (req, res) => {
    ...
    try {
        ...
        if (contactInfo.email) {
            ...
            const ownerContactLink = `${host}/saveContact.html?userId=${userId}`;
            const contactName = contactInfo.name || '';
            const metAt = contactInfo.howWeMet || '';
            const dayString = new Date().toLocaleDateString();

            // Get email template (ownerContactLink set to null to match user's preference - removed card link)
